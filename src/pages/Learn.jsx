...
const Learn = () => {
  ...
  const filteredLearns = learns.filter((learn) => {
    return (
      (filter === 'All' || learn.level.toLowerCase() === filter.toLowerCase()) &&
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {filteredLearns.slice(0,displayedLearns).map((learn, index) => (
            ...
            >
              ...
              <h2 className="text-xl my-3 text-white font-semibold">{learn.title}</h2>
              <p className="text-gray-400 px-3 text-sm leading-7">
                {learn.desc.length > 130
                  ? `${learn.desc.substring(0, 130)}...`
                  : learn.desc}
              </p>
              <p className={`p-2 font-bold capitalize my-4 rounded-md tracking-wider ${levelColors[learn.level.toLowerCase()].color} ${levelColors[learn.level.toLowerCase()].bgColor}`}>
                    {learn.level}
                  </p>
              <button className='text-white py-2 px-4 border rounded-md mt-4 hover:bg-slate-800'>Explore</button>
            </div>
          ))}
        </div>
        )}
